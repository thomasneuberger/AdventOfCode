...
public class Inputs
{
    ...
    public static IReadOnlyList<Game> ParseGames(string input)
    {
        ...
        foreach (var line in lines)
        {
            ...
            var id = int.Parse(gameInfo[0].Substring(gameInfo[0].IndexOf(' ') + 1));

            var draws = gameInfo[1].Split(';', StringSplitOptions.TrimEntries)
                .Select(d =>
                {
                    var draw = new Game.Draw();
                    var colors = d.Split(',', StringSplitOptions.TrimEntries);
                    foreach (var color in colors)
                    {
                        var number = int.Parse(color.Substring(0, color.IndexOf(' ')));
                        switch (color.Substring(color.IndexOf(' ') + 1))
                        {
                            case "red":
                                draw.Red += number;
                                break;
                            ...
                            case "green":
                                ...
                                break;
                            default:
                                throw new Exception($"Color {color} not recognized.");
                        }
                    }
                    return draw;
                })
