...
public static class Inputs
{
    ...
    public static Almanach ParseInput(string input)
    {
        ...
        for (int i = 0; i < seeds.Length; i += 2)
        {
            ...
            almanach.SeedRanges.Add(new SeedRange
            {
                From = from,
                To = to,
                Length = length
        ...
        almanach.SeedToSoil = ParseMappings(lines, "seed-to-soil map:");
        Console.WriteLine("Parsed seeds to soil.");
        almanach.SoilToFertilizer = ParseMappings(lines, "soil-to-fertilizer map:");
        Console.WriteLine("Parsed soil to fertilizer.");
        almanach.FertilizerToWater = ParseMappings(lines, "fertilizer-to-water map");
        Console.WriteLine("Parsed fertilizer to .water");
        almanach.WaterToLight = ParseMappings(lines, "water-to-light map:");
        Console.WriteLine("Parsed water to light.");
        almanach.LightToTemperature = ParseMappings(lines, "light-to-temperature map:");
        Console.WriteLine("Parsed light to humidity.");
        almanach.TemperatureToHumidity = ParseMappings(lines, "temperature-to-humidity map:");
        Console.WriteLine("Parsed humidity to location.");
        almanach.HumidityToLocation = ParseMappings(lines, "humidity-to-location map");

        return almanach;
    }

    private static string[] FindMapLines(string[] lines, string startLine)
    {
        return lines
            .SkipWhile(l => !l.Contains(startLine))
            .Skip(1)
            .TakeWhile(l => !l.Trim().EndsWith(':'))
            .ToArray();
    }

    private static IReadOnlyList<Mapping> ParseMappings(string[] lines, string startLine)
    {
        var mappingLines = FindMapLines(lines, startLine);
        return ParseMappings(mappingLines);
    }

    private static IReadOnlyList<Mapping> ParseMappings(IEnumerable<string> lines)
    {
        var mappings = lines
            .Select(line =>
            {
